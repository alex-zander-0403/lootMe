import * as React from 'react';
import Box from '@mui/material/Box';
import Button from '@mui/material/Button';
import Typography from '@mui/material/Typography';
import Modal from '@mui/material/Modal';
import { TextField } from '@mui/material';
import useBoxes from '../../hooks/useBoxes';

const style = {
  position: 'absolute',
  top: '50%',
  left: '50%',
  transform: 'translate(-50%, -50%)',
  width: 400,
  bgcolor: 'background.paper',
  border: '2px solid #000',
  boxShadow: 24,
  p: 4,
};

export default function AddBox({ open, setOpen }): JSX.Element {
  //   const [open, setOpen] = React.useState(false);
  const handleOpen = () => setOpen(true);
  const handleClose = () => setOpen(false);
  const { submitHandler } = useBoxes();

  return (
    <div>
      <Button onClick={handleOpen}>Open modal</Button>
      <Modal
        open={open}
        onClose={handleClose}
        aria-labelledby="modal-modal-title"
        aria-describedby="modal-modal-description"
      >
        <>
          <Box sx={style} onSubmit={submitHandler} component="form" noValidate autoComplete="off">
            <Typography id="modal-modal-title" variant="h6" component="h2">
              Добавить новый лутбокс
            </Typography>
            {/* <Typography id="modal-modal-description" sx={{ mt: 2 }}>
            Duis mollis, est non commodo luctus, nisi erat porttitor ligula.
          </Typography> */}

            <TextField name="title" id="outlined-basic" label="title" variant="outlined" />
            <TextField name="img" id="outlined-basic" label="img" variant="outlined" />
            <TextField name="price" id="outlined-basic" label="price" variant="outlined" />
            <Button type="submit">Создать</Button>
          </Box>
        </>
      </Modal>
    </div>
  );
}
